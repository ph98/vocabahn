...
docker compose -f docker-compose.prod.yml exec -T api pnpm run ingest:lexicon -- --force
ok "ingest-lexicon.ts has completed!"

# 2. Run seed:dictionary --reset
log "Seeding dictionary (top 10000 lemmas)..."
docker compose -f docker-compose.prod.yml exec -T api pnpm run seed:dictionary --top 10000 --reset
ok "Dictionary seeded."

# 3. Run seed:course
log "Seeding courses..."
docker compose -f docker-compose.prod.yml exec -T api pnpm run seed:course
ok "Courses seeded."

# 4. Run seed:cefr-courses
log "Seeding CEFR courses..."
docker compose -f docker-compose.prod.yml exec -T api pnpm run seed:cefr-courses
